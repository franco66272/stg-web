...
// Identity stabilizer. MediaPipe's handedness is used as the anatomical anchor,
// but a temporary label flip is rejected until it persists for several frames.
...
function predicted(t,now){if(!t.initialized||!t.trackWrist)return null;const dt=Math.min(Math.max((now-t.trackTime)/1000,0),.18);return{x:t.trackWrist.x+t.trackVelocity.x*dt,y:t.trackWrist.y+t.trackVelocity.y*dt}}

function updateTrack(t,lm,now){
  const w=wrist(lm); if(!w)return;
  if(t.trackWrist&&t.trackTime){
    const dt=Math.max((now-t.trackTime)/1000,1/240);
    const vx=(w.x-t.trackWrist.x)/dt,vy=(w.y-t.trackWrist.y)/dt;
    t.trackVelocity.x=t.trackVelocity.x*.35+vx*.65;
    t.trackVelocity.y=t.trackVelocity.y*.35+vy*.65;
  }
  t.trackWrist=w;t.trackTime=now;t.initialized=true;
  t.history.push({x:w.x,y:w.y,t:now});
  t.history=t.history.filter(p=>p.t>=now-260);
}
...
function assignHands(detections,labels,now){
  ...
  // Initial assignment: trust MediaPipe's anatomical handedness when it is unambiguous.
  if(!L.initialized&&!R.initialized){
    if(c.length===1){
      if(c[0].cat==='Right')return{left:null,right:c[0].lm};
      return{left:c[0].lm,right:null};
    }
  ...
  if(c.length===1){
    const q=c[0],lp=predicted(L,now),rp=predicted(R,now);
    // With one hand visible, the stable anatomical label wins unless it is absent.
    if(q.cat==='Left'&&L.initialized)return{left:q.lm,right:null};
    if(q.cat==='Right'&&R.initialized)return{left:null,right:q.lm};
    const dl=distance(q.w,lp),dr=distance(q.w,rp);
    return dl<=dr?{left:q.lm,right:null}:{left:null,right:q.lm};
  }

  const a=c[0],b=c[1];
  const lp=predicted(L,now)||a.w,rp=predicted(R,now)||b.w;
  ...
  const hasClearLabels=!!leftByLabel&&!!rightByLabel;

  if(!hasClearLabels){
    return positionAssignment==='ab'?{left:a.lm,right:b.lm}:{left:b.lm,right:a.lm};
  }

  // If MediaPipe's label assignment and temporal position disagree strongly,
  // keep the previous trajectory for a few frames. This prevents simultaneous
  // arm motion from instantly exchanging the two sabers.
  ...
  const conflict=labelCost>positionCost+0.14;

  if(conflict){
    const side=leftByLabel===a?'a':'b';
    if(identity.labelConflictSide===side)identity.labelConflictStreak++;else{identity.labelConflictSide=side;identity.labelConflictStreak=1}
  }else{
    identity.labelConflictStreak=0;identity.labelConflictSide=null;
  }

  // For the first few conflicting frames, preserve trajectory. Only a sustained
  // reversal is allowed to override it.
  if(conflict&&identity.labelConflictStreak<5){
    return positionAssignment==='ab'?{left:a.lm,right:b.lm}:{left:b.lm,right:a.lm};
  }
...
function filterHand(h,lm,now){
  const s=Number(smoothingEl.value);
  const p=lm.map((q,i)=>({x:h.filters[i].x.filter(q.x,now),y:h.filters[i].y.filter(q.y,now),z:h.filters[i].z.filter(q.z??0,now)}));
  if(h.last&&h.lastTime){
    const dt=Math.max((now-h.lastTime)/1000,1/240);
    for(let i=0;i<p.length;i++){
      h.velocity[i].x=(p[i].x-h.last[i].x)/dt;
      h.velocity[i].y=(p[i].y-h.last[i].y)/dt;
      h.velocity[i].z=(p[i].z-h.last[i].z)/dt;
      if(s&&Math.hypot(h.velocity[i].x,h.velocity[i].y)>1){
        p[i].x=h.last[i].x+(p[i].x-h.last[i].x)*Math.min(1,s*1.8);
        p[i].y=h.last[i].y+(p[i].y-h.last[i].y)*Math.min(1,s*1.8);
      }
    }
  }
...
function predictPoints(h,now){
  ...
  const age=now-h.lostAt,max=Number(predictionEl.value);
  if(age>max)return null;
  const dt=age/1000,decay=Math.pow(Math.max(0,1-age/Math.max(max,1)),1.35);
...
function saber(p,label){
  if(!p)return;
  const w=p[0],q={x:(p[8].x+p[12].x)/2,y:(p[8].y+p[12].y)/2},dx=q.x-w.x,dy=q.y-w.y,len=Math.hypot(dx,dy)||1,e={x:w.x+dx/len*.18,y:w.y+dy/len*.18};
  ctx.save();ctx.lineCap='round';ctx.lineWidth=11;ctx.strokeStyle=label==='left'?'#ff214d':'#2677ff';ctx.shadowBlur=20;ctx.shadowColor=ctx.strokeStyle;ctx.beginPath();ctx.moveTo(w.x*canvas.width,w.y*canvas.height);ctx.lineTo(e.x*canvas.width,e.y*canvas.height);ctx.stroke();ctx.restore();
}
function human(p){
  if(!p||p.length<29)return false;
  const r=[0,11,12,23,24,25,26,27,28],v=r.filter(i=>p[i]&&(p[i].visibility??1)>.35).length;if(v<7)return false;
  const n=p[0],ls=p[11],rs=p[12],lh=p[23],rh=p[24],sw=Math.hypot(ls.x-rs.x,ls.y-rs.y),hw=Math.hypot(lh.x-rh.x,lh.y-rh.y),torso=Math.hypot((ls.x+rs.x)/2-(lh.x+rh.x)/2,(ls.y+rs.y)/2-(lh.y+rh.y)/2),ll=Math.hypot(p[25].x-p[27].x,p[25].y-p[27].y),rl=Math.hypot(p[26].x-p[28].x,p[26].y-p[28].y);
  if(sw<.045||hw<.025||torso<.06||n.y>(ls.y+rs.y)/2+.08||lh.y<(ls.y+rs.y)/2-.05||rh.y<(ls.y+rs.y)/2-.05||ll<.06||rl<.06)return false;
  return hw/sw>.25&&hw/sw<1.35;
}
...
async function loadModels(){
  ...
  handLandmarker=await HandLandmarker.createFromOptions(f,{baseOptions:{modelAssetPath:'https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task',delegate:'GPU'},runningMode:'VIDEO',numHands:2,minHandDetectionConfidence:.2,minHandPresenceConfidence:.2,minTrackingConfidence:.2});
...
function frame(now){
  ...
  try{hr=handLandmarker.detectForVideo(video,now)}catch(e){console.warn(e)}
  const a=assignHands(hr.landmarks||[],hr.handedness||[],now),lr=a.left,rr=a.right;
  if(lr)updateTrack(hands.left,lr,now);if(rr)updateTrack(hands.right,rr,now);
  const left=state(hands.left,lr,now),right=state(hands.right,rr,now);
  let pose=poseLast;poseTick++;
  if(poseTick%4===0){try{const pr=poseLandmarker.detectForVideo(video,now),c=pr.landmarks?.find(human)||null;pose=c?smoothPose(c,now):poseLast}catch(e){console.warn(e)}}
